...
function testPolyLine(wasabeeLink, realLink, operation) {
  if (greatCircleArcIntersect(realLink, wasabeeLink)) {
    ...
    for (const marker of operation.markers) {
      if (
        marker.type == WasabeeMarker.markerTypes.MARKER_TYPE_DESTROY ||
        marker.type == WasabeeMarker.markerTypes.MARKER_TYPE_VIRUS ||
        marker.type == WasabeeMarker.markerTypes.MARKER_TYPE_DECAY
      ) {
        if (
